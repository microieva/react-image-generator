...
export const useCancellableGeneration = () => {
    ...
    const generate = useCallback(async (prompt: string): Promise<GenerationResponse | null> => {
    if (cancelTokenSource.current) {
        cancelTokenSource.current.cancel('New generation started');
    }
    ...
    try {
        ...
        setState(prev => ({ ...prev, taskId: response.data.task_id }));
        // TO DO show progress
        // const status =  await axios.get(`http://127.0.0.1:8000/status/${response.data.task_id}`);
        // setState(prev => ({ ...prev, progress: status.data.progress }));
...
const cancel = useCallback(async () => {
    if (!state.taskId) {
        if (cancelTokenSource.current) {
            cancelTokenSource.current.cancel('Cancelled by user');
        }
        return;
    }
    
    try {
        await axios.post('http://127.0.0.1:8000/cancel-generation', { 
            task_id: state.taskId 
        });
    ...
    // }
        ...
        setState(prev => ({ ...prev, cancelled: false, loading: false, error: `${error}` }));
    }
}, [state.taskId]);
